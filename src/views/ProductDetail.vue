...
const product = computed(() => route.state?.product ?? products.find(p => String(p.id)===String(route.params.id)))
const tabs = [
...
<template>
  <section class="mb-10">
    <div class="bg-F9F1E7 py-8">
      <div class="container">
        <breadcrumb/>
      </div>
    </div>
    <div class="container">
      <div v-if="product" class="flex gap-8 xl:gap-16">
        <div class="flex">
          <div class="w-20">
            <Swiper
                :modules="modules"
                ...
                class="pb-6 min-w-0"
            >
              <swiper-slide v-for="img in product" class="mb-8">
                <img :src="img.url" :alt="img.name" class="w-full max-w-xl mb-4">
              </swiper-slide>
            </Swiper>
          </div>
          <img :src="product.thumb" :alt="product.name" class="w-full max-h-[500px] max-w-xl mb-4">
        </div>
        <div>
          <h2 class="text-3xl font-bold mb-4">{{ product.name }}</h2>
          <div class="text-747474">{{ product.price }}</div>
          ...
          <div class="flex gap-3">
            ...
            <button type="submit" class="border rounded-lg">
              Add to cart
            </button>
            <button type="submit" class="border rounded-lg">
              <span>+</span>
          ...
          <div class="border-t border-3A3A3A">
            <ul>
              <li class="flex">
                <div>SKU</div>
                <div>:{{product.sku}}</div>
        ...
        <template #desc>
          <div class="text-747474">
            ...
            <div class="flex flex-wrap justify-center gap-8">
              <img class="w-full lg:max-w-[calc((100%-32px)/2)] rounded-2xl overflow-hidden" src="/images/banner-hero.jpg" alt="banner">
              <img class="w-full lg:max-w-[calc((100%-32px)/2)] rounded-2xl overflow-hidden" src="/images/banner-hero.jpg" alt="banner">
            </div>
          </div>
        </template>
