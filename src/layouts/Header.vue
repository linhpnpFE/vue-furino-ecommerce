<template>
    ...
    <div ref="menuRef"
      ...
      <div class="py-8" @click.capture="navShow = false">
        <div class="flex justify-center gap-8 mb-8">
          ...
          <div class="xl:hidden size-6 text-black hover:text-B88E2F relative" @click="ui.toggleCart()">
            <div
                v-show="totalProduct"
                class="absolute -right-3 -top-3 min-w-5 h-5 px-1 rounded-full bg-B88E2F text-white
           text-[10px] leading-5 text-center font-bold shadow"
        ...
        <nav class="flex flex-col items-center gap-10 ">
          <router-link to="/" class="text-base font-medium hover:text-B88E2F">Shop</router-link>
          <router-link to="/shop" class="text-base font-medium hover:text-B88E2F">About</router-link>
          <router-link to="/about" class="text-base font-medium hover:text-B88E2F">Contact</router-link>
          <router-link to="/about" class="text-base font-medium hover:text-B88E2F">Blog</router-link>
        </nav>
      </div>
    </div>
  </header>
  <div
      v-if="navShow"
      @click="navShow = false"
      class="fixed inset-0 bg-black/40 backdrop-blur-sm z-40 xl:hidden"
  ></div>
</template>
<script setup>
...
const totalProduct = computed(() => cart.count)
const badgeText = computed(() => totalProduct.value > 99 ? '99+' : String(totalProduct.value))

const navShow = ref(false)
const menuRef = ref(null)

const menuHeight = computed(() => {
  return navShow.value ? `${menuRef.value?.scrollHeight || 0}px` : '0px'
})
